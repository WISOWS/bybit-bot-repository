...
from research_search_breakout import parse_ema_pairs
from research_strategies import MomentumExpansionParams, make_momentum_expansion_strategy
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default=os.path.join(BASE_DIR, "research_search_momentum"))
    parser.add_argument("--initial-balance", type=float, default=10_000.0)
    parser.add_argument("--split-ratio", type=float, default=0.7)
    parser.add_argument("--breakout-lookbacks", default="12,20")
    parser.add_argument("--compression-lookbacks", default="6,10")
    parser.add_argument("--atr-ratios", default="0.80,0.90")
    parser.add_argument("--rr-targets", default="3.0,3.5")
    parser.add_argument("--stop-buffers", default="0.10")
    parser.add_argument("--max-stop-multipliers", default="1.0")
    ...
    parser.add_argument("--swing-lookbacks", default="4")
    ...
    if not loaded_data:
        raise ValueError("Не удалось загрузить OHLCV для momentum-исследования")
    ...
    breakout_lookbacks = [int(value) for value in args.breakout_lookbacks.split(",") if value.strip()]
    compression_lookbacks = [int(value) for value in args.compression_lookbacks.split(",") if value.strip()]
    atr_ratios = [float(value) for value in args.atr_ratios.split(",") if value.strip()]
    ...
    body_fractions = [float(value) for value in args.body_fractions.split(",") if value.strip()]
    range_expansions = [float(value) for value in args.range_expansions.split(",") if value.strip()]
    swing_lookbacks = [int(value) for value in args.swing_lookbacks.split(",") if value.strip()]
    ...
    for (
        ...
        if compression_lookback <= 0 or breakout_lookback <= 0:
            continue
        params = MomentumExpansionParams(
            breakout_lookback_1h=breakout_lookback,
            compression_lookback_4h=compression_lookback,
            ...
            swing_lookback_1h=swing_lookback,
        )
        strategy_fn = make_momentum_expansion_strategy(params)

        per_symbol: Dict[str, Dict[str, Any]] = {}
        for symbol, payload in loaded_data.items():
            rows, _ = run_strategy_backtest_klines(
                strategy_name="momentum_volatility_expansion_search",
    ...
    summary_path = os.path.join(args.output_dir, "momentum_search.json")
